...
import { verificarAcceso, obtenerActasAdmin, obtenerDescargosAdmin, obtenerPagosAdmin, resolverDescargo, conciliarPago } from "./actions/admin"
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        :root {
          --azul-loreto: #0B4A82; --celeste-loreto: #00B2D6; --rojo-loreto: #EB2128;
          --papel: #FFFFFF; --papel-alto: #F8F9FA; --tinta: #212529; --tinta-suave: #495057;
          --linea: #DEE2E6; --radius-s: 4px; --radius-m: 10px; --maxw: 1180px;
        }
        * { box-sizing: border-box; } html { scroll-behavior: smooth; }
        body { margin: 0; background: var(--papel); color: var(--tinta); font-family: 'Public Sans', system-ui, sans-serif; line-height: 1.55; }
        h1, h2, h3, h4 { font-family: 'Fraunces', Georgia, serif; color: var(--azul-loreto); margin: 0 0 0.5em; line-height: 1.15; font-weight: 600; }
        a { color: inherit; } .wrap { max-width: var(--maxw); margin: 0 auto; padding: 0 24px; }
        .topbar { background: var(--azul-loreto); color: #FFFFFF; font-size: 13.5px; }
        ...
        .topbar a { text-decoration: none; opacity: .9; } .topbar a:hover { opacity: 1; text-decoration: underline; }
        ...
        .brand__logo { height: 55px; width: auto; flex: none; }
        .brand__text .eyebrow { font-family: 'IBM Plex Mono', monospace; font-size: 11px; letter-spacing: .08em; text-transform: uppercase; color: var(--rojo-loreto); margin: 0 0 2px; }
        .brand__text strong { display: block; font-family: 'Fraunces', serif; font-weight: 600; font-size: 17px; color: var(--azul-loreto); line-height: 1.2; }
        ...
        nav.primary a { text-decoration: none; font-weight: 600; font-size: 14.5px; color: var(--tinta); padding: 6px 2px; border-bottom: 2px solid transparent; cursor: pointer; }
        nav.primary a:hover { border-color: var(--rojo-loreto); color: var(--azul-loreto); }
        .btn { display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 11px 20px; border-radius: var(--radius-s); font-weight: 700; font-size: 14.5px; text-decoration: none; border: 1.5px solid transparent; cursor: pointer; font-family: 'Public Sans', sans-serif; }
        .btn--primary { background: var(--celeste-loreto); color: #fff; } .btn--ghost { background: transparent; color: var(--azul-loreto); border-color: var(--azul-loreto); }
        .btn--ghost:hover { background: var(--azul-loreto); color: #fff; } .btn--sm { padding: 8px 14px; font-size: 13.5px; } .btn--block { width: 100%; }
        .btn--success { background: #10B981; color: white; } .btn--danger { background: #EF4444; color: white; }
        .hero { padding: 64px 0 56px; background: radial-gradient(circle at 88% 15%, rgba(0, 178, 214, 0.08), transparent 45%), var(--papel-alto); border-bottom: 1px solid var(--linea); }
        .hero .wrap { display: grid; grid-template-columns: 1.15fr 0.85fr; gap: 56px; align-items: center; }
        .hero .eyebrow { font-family: 'IBM Plex Mono', monospace; font-size: 12.5px; letter-spacing: .09em; text-transform: uppercase; color: var(--celeste-loreto); margin-bottom: 14px; font-weight: 500; }
        .hero h1 { font-size: clamp(30px, 4vw, 44px); max-width: 14ch; }
        .hero p.lead { font-size: 17.5px; color: var(--tinta-suave); max-width: 46ch; margin: 14px 0 28px; }
        section { padding: 72px 0; } .section-head { max-width: 60ch; margin-bottom: 40px; }
        .section-head .kicker { font-family: 'IBM Plex Mono', monospace; font-size: 12px; letter-spacing: .08em; text-transform: uppercase; color: var(--rojo-loreto); margin-bottom: 10px; font-weight: 500; }
        ...
        .art-card { background: var(--papel); padding: 30px 26px; } .art-card__tag { font-family: 'IBM Plex Mono', monospace; font-size: 12px; color: var(--celeste-loreto); font-weight: 600; margin-bottom: 14px; display: block; }
        .consulta-panel { background: var(--azul-loreto); color: #F8F9FA; border-radius: var(--radius-m); padding: 40px; display: grid; grid-template-columns: 1fr 1fr; gap: 40px; align-items: center; }
        .consulta-form { background: var(--papel); border-radius: var(--radius-m); padding: 26px; color: var(--tinta); }
        .field { margin-bottom: 16px; } .field label { display: block; font-size: 13px; font-weight: 600; margin-bottom: 6px; color: var(--tinta); }
        .field input, .field textarea { width: 100%; padding: 11px 12px; border: 1.5px solid var(--linea); border-radius: var(--radius-s); font-family: 'IBM Plex Mono', monospace; font-size: 14px; background: #fff; color: var(--tinta); }
        .admin-table { width: 100%; text-align: left; border-collapse: collapse; background: #fff; border-radius: var(--radius-m); overflow: hidden; border: 1px solid var(--linea); box-shadow: 0 2px 8px rgba(0,0,0,0.05); }
        .admin-table th { background: var(--papel-alto); padding: 16px; font-weight: 600; border-bottom: 2px solid var(--linea); font-size: 14px; color: var(--azul-loreto); }
        .admin-table td { padding: 16px; border-bottom: 1px solid var(--linea); font-size: 14.5px; }
                            ...
                            <tr key={item.id}>
                              ...
                              <td>
                                <span className="badge" style={{
                                  background: item.estado === 'PENDIENTE' || item.estado === 'PRESENTADO' || item.estado === 'PENDIENTE_CONCILIACION' ? 'rgba(245, 158, 11, 0.15)' : 'rgba(16, 185, 129, 0.15)',
                                  color: item.estado === 'PENDIENTE' || item.estado === 'PRESENTADO' || item.estado === 'PENDIENTE_CONCILIACION' ? '#B45309' : '#047857'
                                }}>{item.estado || item.dniTitular}</span>
                              </td>
                              ...
                              <td>
                                {vista !== 'admin_actas' ? (
                                  ...
                                  <span style={{fontSize: '13px', color: 'gray'}}>Módulo de Carga</span>
                                )}
                              </td>
                            </tr>
                          ))}
        ...
        <div className="modal-overlay" onClick={() => setItemModal(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{background: 'var(--papel-alto)', padding: '15px', borderRadius: '6px', marginBottom: '20px', fontSize: '14px'}}>
              <p><strong>Acta Asociada:</strong> {itemModal.infraccion?.nroActa}</p>
              <p><strong>DNI Titular:</strong> {itemModal.infraccion?.dniTitular}</p>
            ...
            <div style={{marginBottom: '20px'}}>
              <a 
                href={vista === 'admin_descargos' ? itemModal.archivosUrl?.[0] : itemModal.comprobanteUrl} 
                target="_blank" 
                rel="noreferrer"
                className="btn btn--ghost btn--block"
              >
                Abrir Documento Adjunto (Nube)
              </a>
            </div>
            ...
            {(itemModal.estado === 'PRESENTADO' || itemModal.estado === 'PENDIENTE_CONCILIACION') ? (
              <div style={{borderTop: '1px solid var(--linea)', paddingTop: '20px'}}>
                ...
                <div style={{display: 'flex', gap: '10px', marginTop: '10px'}}>
                  {vista === 'admin_descargos' ? (
                    <>
                      <button onClick={() => auditarDescargo('RESUELTO_A_FAVOR')} disabled={procesando} className="btn btn--success" style={{flex: 1}}>Fallo a Favor (Anular)</button>
                      <button onClick={() => auditarDescargo('RECHAZADO')} disabled={procesando} className="btn btn--danger" style={{flex: 1}}>Rechazar Descargo</button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => auditarPago('CONCILIADO')} disabled={procesando} className="btn btn--success" style={{flex: 1}}>Aprobar Pago</button>
                      <button onClick={() => auditarPago('RECHAZADO')} disabled={procesando} className="btn btn--danger" style={{flex: 1}}>Rechazar Comprobante</button>
                    </>
                  )}
                </div>
              </div>
      ...
      <footer className="site" style={{background: 'var(--azul-loreto)', color: '#F8F9FA', padding: '40px 0'}}>
        <div className="wrap" style={{textAlign: 'center', fontSize: '14px'}}>
          © 2026 Juzgado de Faltas Municipal de Loreto — Todos los derechos reservados
        </div>
      </footer>
    </>
  )
